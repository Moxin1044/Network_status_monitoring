...
BIN_NAME="network_status_monitoring"
...
do_install() {
    ...
    echo -e "${BOLD}  步骤 1/5: 检查可执行文件${NC}"
    echo ""

    if [[ ! -f "$SCRIPT_DIR/$BIN_NAME" ]]; then
        error "未找到可执行文件: $SCRIPT_DIR/$BIN_NAME"
        exit 1
    fi

    if [[ ! -x "$SCRIPT_DIR/$BIN_NAME" ]]; then
        chmod +x "$SCRIPT_DIR/$BIN_NAME"
    fi

    success "可执行文件已就绪: $SCRIPT_DIR/$BIN_NAME"

    echo ""
    echo -e "${BOLD}  步骤 2/5: 安装文件${NC}"
    echo ""
    ...
    sudo cp "$SCRIPT_DIR/$BIN_NAME" "$INSTALL_DIR/"
    ...
    if [[ "$KEEP_CONFIG" -eq 1 ]]; then
        ...
        if [[ -f "$SCRIPT_DIR/config.yml" ]]; then
            sudo cp "$SCRIPT_DIR/config.yml" "$INSTALL_DIR/"
        else
    ...
    success "可执行文件已安装: ${BIN_LINK} -> ${INSTALL_DIR}/${BIN_NAME}"
    success "配置文件: ${INSTALL_DIR}/config.yml"

    echo ""
    echo -e "${BOLD}  步骤 3/5: 创建 systemd 服务${NC}"
    echo ""
