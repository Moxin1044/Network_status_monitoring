...
set "APP_NAME=network-status-monitoring"
...
if exist "%INSTALL_DIR%\%APP_NAME%.exe" set "FOUND=1"
...
if %ERRORLEVEL% neq 0 (
    echo   [✖] 未找到 cargo
    echo.
    set /p "install_rust=  是否自动安装 Rust? (y/n): "
    if /i "!install_rust!"=="y" (
        echo   [i] 正在安装 Rust...
...
copy /y "target\release\%APP_NAME%.exe" "%INSTALL_DIR%\" >nul

if "!KEEP_CONFIG!"=="1" (
    echo   [i] 保留现有 config.yml
) else (
    if exist "config.yml" (
        copy /y "config.yml" "%INSTALL_DIR%\" >nul
    ) else (
        (
            echo feishu_webhook: "*"
...
if "!USE_NSSM!"=="1" (
    ...
    nssm install %SERVICE_NAME% "%INSTALL_DIR%\%APP_NAME%.exe"
    ...
    schtasks /delete /tn "%SERVICE_NAME%" /f >nul 2>&1
    schtasks /create /tn "%SERVICE_NAME%" /tr "\"%INSTALL_DIR%\%APP_NAME%.exe\"" /sc onstart /ru SYSTEM /rl highest /f
